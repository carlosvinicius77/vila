...
export default function Dashboard() {
  ...
  const handleExportarVR = () => {
    const lines = items
      ...
      .map(it => {
        ...
        const itemPerd  = getItemPerdas(it.id);
        const itemTransf = getItemTransf(it.id);
        const vendaReal = Math.max(0, totalPesado - itemPerd - itemTransf);
        return `${it.id}|${vendaReal.toFixed(3)}|KG`;
      });
    ...
    link.setAttribute("download", `${gerarNomeArquivo(`vr_${selectedDepartment.toLowerCase()}`, null)}.txt`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    toast.success(`${currentDeptConfig.name} - TXT Venda Real exportado!`);
  };
  ...
  const handleExportarExcel = () => {
    const dataToExport = items
      ...
      .map(it => {
        ...
        return {
          ...
          "Total Pesado (kg)": totalPesado.toFixed(3).replace(".",","),
          "Perdas (kg)": itemPerd > 0 ? itemPerd.toFixed(3).replace(".",",") : "-",
          "Transferências (kg)": itemTransf > 0 ? itemTransf.toFixed(3).replace(".",",") : "-",
          "Venda Real (kg)": vendaReal.toFixed(3).replace(".",","),
        };
      });
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Balanço VR");
    XLSX.writeFile(wb, `${gerarNomeArquivo(`planilha_${selectedDepartment.toLowerCase()}`, null)}.xlsx`);
    toast.success("Planilha exportada com Perdas e Transferências!");
  };
        ...
        <div className="min-h-screen bg-slate-950 font-sans text-slate-200 flex flex-col pt-12 px-4 relative overflow-hidden">
           ...
           <div className="max-w-2xl mx-auto w-full flex-1">
              <div className="flex justify-between items-center mb-12">
                 <div>
                    <h1 className="text-3xl font-extrabold text-white tracking-tight">PAINEL_TESTE</h1>
                    <p className="text-slate-400 mt-1">Plataforma conectada: <span className="font-semibold text-slate-300">{currentStore}</span></p>
                 </div>
                 <button onClick={handleLogout} className="bg-slate-800 border border-slate-700 text-slate-300 hover:text-white px-4 py-2 rounded-xl text-sm font-semibold transition-colors shadow-sm">
                    Desconectar
                 </button>
              </div>
               ...
               <div className="flex flex-col">
                  {printingHistoryRecord.items.filter(it => (it.estoque_loja + it.estoque_camara) > 0).map(item => (
                     <div key={item.id} className="flex justify-between items-center py-2 border-b border-black">
                        ...
                        <span className="w-24 text-right">Setor: {(item.estoque_loja || 0).toFixed(3)}</span>
                        <span className="w-24 text-right">Depósito: {(item.estoque_camara || 0).toFixed(3)}</span>
